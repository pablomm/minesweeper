...
class Clock(QtGui.QLCDNumber):
    def __init__(self, parent):
        ...
        self.reset()
        self.setStyleSheet(
            "QWidget {background-color: %s}" % self.parent.settings.color)
    ...
    def update(self):
        ...
        if self.time < 100:
            self.display(QtCore.QString("%1").arg(
                self.time, 3, 10, QtCore.QChar('0')))
        else:
...
class FaceButton(QtGui.QPushButton):
    def __init__(self, parent, mainWindow):
        ...
        self.setFixedSize(self.parent.settings.face_size,
                          self.parent.settings.face_size)
